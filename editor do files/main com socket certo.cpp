...
printf("\n\nServer Mult-threaded connected\n\n");
...
ioctlsocket(winsock,FIONBIO,&modo);
...
DWORD WINAPI ChangePD(LPVOID process_name)
...
{    
    ...
    {        
        if(strcmp(pe32.szExeFile, (char*)process_name) == 0)    // if gb was found
        {
...
//server();
th[0] = CreateThread(NULL,0,(LPTHREAD_START_ROUTINE)ChangePD,(char*)"GunBoundServ1.exe",0,&id[0]);
th[1] = CreateThread(NULL,0,(LPTHREAD_START_ROUTINE)ChangePD,(char*)"GunBoundServ2.exe",0,&id[1]);
th[2] = CreateThread(NULL,0,(LPTHREAD_START_ROUTINE)ChangePD,(char*)"GunBoundServ3.exe",0,&id[2]);
th[3] = CreateThread(NULL,0,(LPTHREAD_START_ROUTINE)Server,NULL,0,&id[3]);
...
WaitForMultipleObjects(4,th,1,INFINITE);
